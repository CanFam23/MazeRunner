...
public class Chunk {
	...
	private static int setChunkNumber(){
		currentNumber++;
	...
	public Chunk(int xDimension, int yDimension, int xPosition, int yPosition) {
		...
		this.ChunkNumber = setChunkNumber();
	}
	...
	public void draw(Graphics2D g) {
		...
		//Draw a Circle in top left of all chunks, used for testing, can delete now or later
...
//		g.setColor(temp);
	...
	public int[][] getCoords(){
		return new int[][] {{xPosition,xPosition+chunkWidth,xPosition+chunkWidth,xPosition},{yPosition,yPosition,yPosition+chunkHeight,yPosition+chunkHeight}};
	}
	
	//Returns a List of integers, which represent what side of the player is colliding with a wall
	public List<Integer> checkCollision() {
		
		List<Integer> collisions = new ArrayList<>();
		
		//Check each wall in the chunk, if a wall chunk and colliding, add it's returned int value to list
	...
	public int collision(Wall w) {
        ...
        final int[] otherXCoords = new int[] {x-HITBOX_BUFFER_AMOUNT,x+WALL_WIDTH+HITBOX_BUFFER_AMOUNT,x+WALL_WIDTH+HITBOX_BUFFER_AMOUNT,x-HITBOX_BUFFER_AMOUNT};
        final int[] otherYCoords = new int[] {y-HITBOX_BUFFER_AMOUNT,y-HITBOX_BUFFER_AMOUNT,y+WALL_HEIGHT+HITBOX_BUFFER_AMOUNT,y+WALL_HEIGHT+HITBOX_BUFFER_AMOUNT};
